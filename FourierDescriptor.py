...
if __name__ == "__main__":
    ...
    fd_mag1, x_rec1, y_rec1 = get_FD_abs(x1, y1, order)
    fd_mag2, x_rec2, y_rec2 = get_FD_abs(x2, y2, order)
    fd_mag6, x_rec6, y_rec6 = get_FD_abs(x6, y6, order)
    ...
    for i in range(len(x_rec1)): 
        plt.plot(y_rec1[i], x_rec1[i])
    for j in range(len(x_rec2)): 
        plt.plot(y_rec2[j], x_rec2[j])
    for k in range(len(x_rec6)): 
        plt.plot(y_rec6[k], x_rec6[k])
    ...
    for i in range(len(fd_mag1)):
        plt.plot(np.arange(2, 2*order), fd_mag1[i][1:-1], 'bo')
    for i in range(len(fd_mag2)):
        plt.plot(np.arange(2, 2*order), fd_mag2[i][1:-1], 'gx')
    for i in range(len(fd_mag6)):
        plt.plot(np.arange(2, 2*order), fd_mag6[i][1:-1], 'r^')
